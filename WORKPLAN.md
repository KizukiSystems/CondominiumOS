...
- **Lane:** A · **Depends on:** open item #11 (POC code made available) · **Status:** `[blocked — needs input]`
...
- **Done when:** running the pipeline on the fixtures yields the 5-item agenda with 100% citation coverage, verified by an automated check
...
- **Done when:** the feeling test passes: a busy manager glancing at the agenda knows, per item, what the decision is and what the options are, within seconds per item
